...
export default Vue.observable({
	...
	getTeamSeason: async function(id, season) {
		...
		let data = await axios.get(
			`https://statsapi.web.nhl.com/api/v1/teams/${id}?expand=team.stats&season=${this.formatSeason(
				season
			)}`
		);
	...
	getTeamRoster: async function(id, season) {
		let data = await axios.get(
			` https://statsapi.web.nhl.com/api/v1/teams/${id}/roster?season=${this.formatSeason(season)}`
		);

		return data;
	},
	getHistoricalRatings: async function(id, range = 30) {
		let thisYear = (await axios.get(`https://statsapi.web.nhl.com/api/v1/seasons/current`)).data
			.seasons[0].seasonId;
	...
	getPlayoffOdds: async function(teamCode) {
		if (!playoffOdds.length) {
			let r = await axios.get("/advanced_stats/playoffs.csv");
			playoffOdds = Papa.parse(r.data, { header: true }).data;
		}

		let found = playoffOdds.find(t => teamCode === t.teamCode && t.scenerio === "ALL");
